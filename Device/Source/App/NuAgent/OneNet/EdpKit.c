...
{									\
    ...
    *ds_id = (char*)malloc(strlen(id)+1);				\
    memcpy((INT8U*)(*ds_id), id, strlen(id)+1);				\
    									\
...
#define UNPACK_SAVE_DATA_TYPE1_END_STRING				\
    ...
    *value = (char*)malloc(strlen(valuestring)+1);			\
    memcpy((INT8U*)(*value), valuestring, strlen(valuestring)+1);	\
    cJSON_Delete(json_obj);						\
    return 0;								\
}
...
#define MKFUN_UNPACK_SAVE_DATA_TYPE2(TYPE, NAME, MED)			\
	...
	*ds_id = (char*)malloc(len);					\
	memcpy((INT8U*)(*ds_id), json_child->string, len);		\
...
static void FormatAt(char* buffer, int len, time_t now)
{
    ...
    strftime(buffer, len, "%F %T", _localtime_r(&now, &timinfo));
#else
	struct timeval localtime;

    GetLocalTime(&localtime);
    sprintf(buffer, "%s", (char*)ctime((const time_t*)&(localtime.tv_sec)));
#endif
}
...
int32 CheckCapacity(Buffer* buf, uint32 len)
{
    ...
    if (cap_len > buf->_capacity)
    {
        ...
        memcpy((INT8U*)pdata, buf->_data, buf->_write_pos);
        free(buf->_data);
        buf->_data = pdata;
...
int32 ReadBytes(EdpPacket* pkg, uint8** val, uint32 count)
{
    ...
    *val = (uint8*)malloc(sizeof(uint8) * count);
    memcpy((INT8U*)*val, pkg->_data + pkg->_read_pos, count);
    pkg->_read_pos += count;
    return 0;
}
...
int32 WriteBytes(Buffer* buf, const void* bytes, uint32 count)
{
    ...
    if (CheckCapacity(buf, count))
        return -1;
    memcpy((INT8U*)(buf->_data + buf->_write_pos), bytes, count);
...
EdpPacket* GetEdpPacket(RecvBuffer* buf)
{
    ...
    /* shrink buffer */
    NST_MoveMemOvlap(buf->_data, buf->_data + flag, buf->_write_pos - flag);
    buf->_write_pos -= flag;
    return pkg;
}
