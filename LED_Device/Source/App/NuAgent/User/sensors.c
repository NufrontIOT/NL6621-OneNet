...
extern unsigned char Attr_flags;
extern struct sys_status_t sys_status;
...
void upload_led_data(unsigned char attr_flags)
{
    int upload_data = 0;

    if(attr_flags == 0) return;

	if(attr_flags&0x01)	{
	   	upload_data = led_data.Switch;
	    SensorData_Upload("onoff", &upload_data, DATATYPE_INT);	
	    OSTimeDly(35);
	}
   	if(attr_flags&0x10)	{
	 	upload_data = get_light_gpio_status(LED_WHITE_GPIO_NUM);
	    SensorData_Upload("white", &upload_data, DATATYPE_INT);
	    OSTimeDly(35);
	}
   	if(attr_flags&0x20)	{
		upload_data = get_light_gpio_status(LED_RED_GPIO_NUM);
	 	SensorData_Upload("red", &upload_data, DATATYPE_INT);
	    OSTimeDly(35);
	}
   	if(attr_flags&0x40)	{
	 	upload_data = get_light_gpio_status(LED_GREEN_GPIO_NUM);
	 	SensorData_Upload("green", &upload_data, DATATYPE_INT);
	    OSTimeDly(35);
	}
    if(attr_flags&0x80)	{
	 	upload_data = get_light_gpio_status(LED_BLUE_GPIO_NUM);
	   	SensorData_Upload("blue", &upload_data, DATATYPE_INT);
	    OSTimeDly(35);
	}
}
