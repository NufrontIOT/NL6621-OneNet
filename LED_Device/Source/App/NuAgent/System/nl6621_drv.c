...
#define  ONENET_CONFIG_DATA_ADDR      (0x65000)  /* about 416k Bytes offset */
...
{
	ReadFlash((UINT8 *)pConfig, sizeof(Onenet_config), ONENET_CONFIG_DATA_ADDR);
...
{
	WriteFlash((UINT8*)pConfig, sizeof(Onenet_config), ONENET_CONFIG_DATA_ADDR);
    return 0;
}

int nl6621_GetSmartLedData(led_status_flash *pConfig)
{
    int length =  sizeof(led_status_flash);
	ReadFlash((UINT8 *)pConfig, length, GAGENT_SMART_LED_DATA_ADDR);
	return 0;
}
...
{
    int length =  sizeof(led_status_flash);
	WriteFlash((UINT8*)pConfig, length, GAGENT_SMART_LED_DATA_ADDR);
    return 0;
}
