 ...
 * TCP server 
 **/
#define TCP_SERVER_PORT              (8200)        /* LAN TCP server port */
...
#define UDP_SERVER_BUFFER_SIZE       (1024)	    	/* LAN TCP server receive size */
static int UDPServerFd = -1;
char * UDPServerRecv_data = NULL;

extern void Agent_cloud_init(void);
extern void lan_softap_config(char *data, int data_len);
extern void onenet_setConfigData(char *buffer,unsigned char length);
...
void NuAgent_ServerFound(const char *name, ip_addr_t *ipaddr, void *arg)
{
...
int NuAgent_GetHostByName(char *domain, char *IPAddress)
{
    unsigned char Err;
    ip_addr_t ip_addr;
    char str[32];
    ...
    switch (dns_gethostbyname(domain, &ip_addr, NuAgent_ServerFound, NULL)) {
        case ERR_OK:
...
int Socket_TCPClientRecvData(char *recvbuf, int len)
{
	...
	setsockopt( TCPClientFd, SOL_SOCKET, SO_RCVBUF, &bufferSize, 4 );
    return recv_count;
}
...
int Socket_UDPServerRecvData(void)
{
	...
	while (1) 
	{
		...
		if (recv_count <= 0) {
		   ...
		   if(recv_count > 30 && UDPServerRecv_data[0] == 'i' && UDPServerRecv_data[1] == 'd' && UDPServerRecv_data[2] == ':')	{
		        onenet_setConfigData(UDPServerRecv_data,recv_count); 

				while(count_send > 0) {
				    count_send--;
				    sendto(UDPServerFd, UDPServerRecv_data, recv_count, 0,
	                        (struct sockaddr_in *) &client_addr, 
	                        sizeof(struct sockaddr_in)); 
					OSTimeDly(80);	
				}
		   } else if(recv_count == 20 && memcmp((INT8U *)UDPServerRecv_data,"resetreset",10) == 0 ) {
				...
				nl6621_Reset();	   
		   	    ...
		   	    lan_softap_config(UDPServerRecv_data,recv_count);	
		   }
		}			
	}	
}
